...
# Palette — light theme (high contrast on white)
GOLD    = '#C87800'   # dark amber
TEAL    = '#0A7E8C'   # dark teal
CORAL   = '#C94030'   # muted red
PURPLE  = '#6248A8'   # medium purple
GREEN   = '#2A8A5E'   # forest green
BLUE    = '#2E6FC4'   # strong blue
ORANGE  = '#C4600A'   # burnt orange
PINK    = '#A8306E'   # berry

BAR_BLUE = '#A8C8E8'  # light steel blue (like reference image)
...
OUTPUT_DIR = 'analysis_plots_light'
...
def plot_price_distribution(df):
    ...
    fig.suptitle('Price Distribution — Sri Lanka Used Car Market', fontsize=16, fontweight='bold', color='#1A1A2E')

    prices = df['Price'].dropna()

    # Histogram
    ax = axes[0]
    n, bins, patches = ax.hist(prices, bins=60, color=BAR_BLUE, edgecolor='#5A9ABF', linewidth=0.4, alpha=0.9)
    ...
    fig.text(0.5, 0.01, stats, ha='center', fontsize=10, color='#666666')
...
def plot_price_by_brand(df):
    ...
    fig.suptitle('Price Analysis by Brand', fontsize=16, fontweight='bold', color='#1A1A2E')

    # Horizontal bar — median price
    ax = axes[0]
    colors = [BRAND_COLORS.get(b, GOLD) for b in brand_stats.index]
    bars = ax.barh(brand_stats.index, brand_stats['median'], color=colors, edgecolor='#DDDDDD', linewidth=0.4)
    for bar, val in zip(bars, brand_stats['median']):
        ax.text(val + 0.5, bar.get_y() + bar.get_height() / 2,
                f'Rs.{val:.1f}L', va='center', fontsize=8.5, color='#222222')
...
def plot_price_by_year(df):
    ...
    fig.suptitle('Price vs Year of Manufacture', fontsize=16, fontweight='bold', color='#1A1A2E')
    ...
    cb.set_label('Year', color='#444444')
    cb.ax.yaxis.set_tick_params(color='#444444')
...
def plot_mileage_vs_price(df):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    fig.suptitle('Mileage vs Price Analysis', fontsize=16, fontweight='bold', color='#1A1A2E')
    ...
    bars = ax.bar(band_stats.index, band_stats.values, color=colors, edgecolor='#DDDDDD', linewidth=0.4)
    for bar, val in zip(bars, band_stats.values):
        ax.text(bar.get_x() + bar.get_width() / 2, val + 0.5,
                f'Rs.{val:.1f}L', ha='center', fontsize=9, color='#222222')
...
def plot_fuel_type(df):
    ...
    fig.suptitle('Fuel Type Analysis', fontsize=16, fontweight='bold', color='#1A1A2E')
    ...
    wedges, texts, autotexts = ax.pie(fuel_counts.values, labels=fuel_counts.index,
                                       ...
                                       wedgeprops=dict(width=0.55, edgecolor='#DDDDDD', linewidth=1.5),
                                       pctdistance=0.75)
    for t in autotexts:
        t.set_color('#222222')
        t.set_fontsize(9)
    ...
    # Bar — count
    ax = axes[1]
    bars = ax.bar(fuel_counts.index, fuel_counts.values, color=fuel_colors, edgecolor='#DDDDDD', linewidth=0.4)
    for bar, val in zip(bars, fuel_counts.values):
        ax.text(bar.get_x() + bar.get_width() / 2, val + 10,
                f'{val:,}', ha='center', fontsize=9, color='#222222')
    ax.set_ylabel('Number of listings')
    ...
    # Bar — median price
    ax = axes[2]
    bars = ax.bar(fuel_price.index, fuel_price.values, color=fuel_colors, edgecolor='#DDDDDD', linewidth=0.4)
...
def plot_gear_type(df):
    ...
    fig.suptitle('Transmission Type Analysis', fontsize=16, fontweight='bold', color='#1A1A2E')
    ...
    w = 0.35
    b1 = ax.bar(x - w / 2, gear_price['median'], w, label='Median', color=GOLD, edgecolor='#DDDDDD')
    b2 = ax.bar(x + w / 2, gear_price['mean'],   w, label='Mean',   color=TEAL, edgecolor='#DDDDDD')
    ax.set_xticks(x)
...
def plot_condition(df):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle('Vehicle Condition vs Price', fontsize=16, fontweight='bold', color='#1A1A2E')
    ...
    bars = ax.bar(counts.index, counts.values,
                  color=cond_colors[:len(counts)], edgecolor='#DDDDDD')
...
def plot_equipment_impact(df):
    ...
    fig.suptitle('Equipment Features — Impact on Price', fontsize=16, fontweight='bold', color='#1A1A2E')
    axes = axes.flatten()
        ...
        for j in range(len(pivot.columns)):
            ...
            if not np.isnan(val):
                ax.text(j, i, f'{val:.0f}', ha='center', va='center',
                        fontsize=8, color='#111111' if val < pivot.values[~np.isnan(pivot.values)].max() * 0.7 else '#EEEEEE')

    cb = plt.colorbar(im, ax=ax)
    cb.set_label('Median Price (LKR Lakhs)', color='#444444')
...
def plot_correlation(df):
    ...
    fig.suptitle('Feature Correlation Heatmap', fontsize=16, fontweight='bold', color='#1A1A2E')
...
def plot_feature_importance(model, feature_names):
    ...
    fig.suptitle('Feature Importance Analysis', fontsize=16, fontweight='bold', color='#1A1A2E')

    # Horizontal bar
    ax = axes[0]
    colors = [GOLD if v > top.quantile(0.75) else TEAL if v > top.quantile(0.5) else '#AAAACC'
              for v in top.values]
    bars = ax.barh(top.index, top.values, color=colors, edgecolor='#DDDDDD', linewidth=0.3)
    ax.set_xlabel('Importance Score')
    ax.set_title('Top 20 Features by Importance')
    for bar, val in zip(bars, top.values):
        ax.text(val + 0.001, bar.get_y() + bar.get_height() / 2,
                f'{val:.3f}', va='center', fontsize=8, color='#222222')
...
def plot_bias_variance(X_train, X_test, y_train, y_test):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(18, 7))
    fig.suptitle('Bias-Variance Tradeoff Analysis', fontsize=16, fontweight='bold', color='#1A1A2E')

    # Grouped bar
    ax = axes[0]
    bar_colors_tr = [GOLD   if f else '#AAAACC' for f in optimal_flags]
    bar_colors_te = [GREEN  if f else TEAL      for f in optimal_flags]
    b1 = ax.bar(x - w / 2, trains, w, label='Train R²', color=bar_colors_tr, edgecolor='#DDDDDD')
    b2 = ax.bar(x + w / 2, tests,  w, label='Test R²',  color=bar_colors_te, edgecolor='#DDDDDD')
    for bar, val in zip(list(b1) + list(b2), trains + tests):
        ax.text(bar.get_x() + bar.get_width() / 2, val + 0.003,
                f'{val:.3f}', ha='center', fontsize=7.5, color='#222222', rotation=90)
    ax.set_xticks(x)
    ...
    ax.annotate('Optimal', xy=(opt_i, tests[opt_i] + 0.03),
                ...
                ha='center', color=GOLD, fontsize=10, fontweight='bold', backgroundcolor='#FFFFFF')
...
def plot_learning_curves(model, X, y):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    fig.suptitle('Learning Curves — Model Generalisation', fontsize=16, fontweight='bold', color='#1A1A2E')
...
def plot_cross_validation(model, X, y):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle('10-Fold Cross-Validation Results', fontsize=16, fontweight='bold', color='#1A1A2E')

    # Bar per fold
    ax = axes[0]
    fold_colors = [GREEN if s > scores.mean() else CORAL for s in scores]
    bars = ax.bar(range(1, 11), scores, color=fold_colors, edgecolor='#DDDDDD', linewidth=0.4)
    ...
    for bar, val in zip(bars, scores):
        ax.text(bar.get_x() + bar.get_width() / 2, val + 0.001,
                f'{val:.3f}', ha='center', fontsize=8.5, color='#222222')
    ax.set_xlabel('Fold')
    ax.set_ylabel('R² Score')
    ...
    # Distribution
    ax = axes[1]
    ax.hist(scores, bins=8, color=GOLD, edgecolor='#DDDDDD', linewidth=0.4, alpha=0.8)
...
def plot_residuals(model, X_test, y_test):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Residual Analysis — Model Error Diagnostics', fontsize=16, fontweight='bold', color='#1A1A2E')
    ...
    # Residual histogram
    ax = axes[0, 1]
    ax.hist(resid, bins=60, color=GOLD, edgecolor='#DDDDDD', linewidth=0.3, alpha=0.85)
...
def plot_prediction_vs_actual(model, X_test, y_test):
    ...
    fig.suptitle('Predicted vs Actual Price', fontsize=16, fontweight='bold', color='#1A1A2E')
    ...
    ax.text(0.05, 0.92,
            ...
            bbox=dict(boxstyle='round', facecolor='#F8F8F8', edgecolor=GOLD, alpha=0.9))
    ...
    bars = ax.bar(range(len(bin_mae)), bin_mae.values, color=colors, edgecolor='#DDDDDD')
...
def plot_price_segments(df):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Market Segment Analysis', fontsize=16, fontweight='bold', color='#1A1A2E')
    ...
    # Count bar
    ax = axes[0, 1]
    bars = ax.bar(labels, seg_counts.values, color=seg_colors, edgecolor='#DDDDDD')
...
def plot_depreciation(df):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(16, 7))
    fig.suptitle('Depreciation Curves by Brand', fontsize=16, fontweight='bold', color='#1A1A2E')
    ...
    for brand, color in zip(top_brands, colors):
        ...
        if len(sub) >= 5 and sub.iloc[0]['Price'] > 0:
            ...
            ax.plot(sub['Car_Age'], sub['pct'], color=color, lw=2,
                    marker='o', markersize=4, label=brand)
    ax.axhline(50, color='#AAAAAA', linestyle='--', lw=1.5, label='50% value retained')
    ax.set_xlabel('Car Age (years)')
